...
정수형 분자(`num`)와 분모(`den`)로 구성된 분수(Fraction)를 추상 자료형(ADT)으로 정의하고, 이를 활용한 사칙연산 계산기입니다.

## 1. 데이터 모델 (Data)
- **구조체 `Fraction`**: `int num` (분자), `int den` (분모)
...
## 4. 컴파일 및 실행
```bash
gcc -o fraction_calc main.c my_fraction.c
./fraction_calc
